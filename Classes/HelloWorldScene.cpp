...
Scene* GetScene1()
{
	...
	listener1->onTouchMoved = [=](Touch* touch, Event* event)
	{
		...
		{
			sprShip->setPositionX(touch->getLocation().x);
		}
		if (touch->getLocation().y < visibleSize.height && touch->getLocation().y > 0)
		{
			sprShip->setPositionY(touch->getLocation().y);
		}
	...
	contactListener->onContactBegin = [=](PhysicsContact& contact) {
		...
		if (nodeA->getTag() == 20 && nodeB->getTag() == 30 || nodeB->getTag() == 20 && nodeA->getTag() == 30)//me and enemyunkillable
		{
			...
			if (nodeA->getTag() == 20) { auto sprSmallBlowUp = SmallBlowUp(nodeA->getPositionX(), nodeA->getPositionY()+28); scene->addChild(sprSmallBlowUp); } else { auto sprSmallBlowUp = SmallBlowUp(nodeB->getPositionX(), nodeB->getPositionY()+28); scene->addChild(sprSmallBlowUp); }
			return false;
		}
		...
		{
			audio->playEffect("explosion.mp3");
			if (nodeA->getTag() > 34 && nodeA->getTag() < 39) { auto sprSmallBlowUp = SmallBlowUpWide(nodeA->getPositionX(), nodeA->getPositionY() - 43); scene->addChild(sprSmallBlowUp); } else { auto sprSmallBlowUp = SmallBlowUpWide(nodeB->getPositionX(), nodeB->getPositionY() - 43); scene->addChild(sprSmallBlowUp); }
			if (nodeA->getTag() != 10) { nodeB->removeFromParentAndCleanup(true); nodeA->setTag(nodeA->getTag() + 1); } else if (nodeB->getTag() != 10) { nodeA->removeFromParentAndCleanup(true); nodeB->setTag(nodeB->getTag() + 1); }
			return false;
		}
